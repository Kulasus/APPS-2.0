...
#include "lcd_lib.h"
#include <string>
#include "font12x20_msb.h"		// Font import
#include <vector>

#define WIDTH 12	// Font width
#define HEIGHT 20	// Font height
...
public:
    ...
    CircleFull( Point2D t_center, int32_t t_radius, RGB t_fg, RGB t_bg ) :
            center( t_center ), radius( t_radius ), GraphElement( t_fg, t_bg ) {
    	this->fg = t_fg;
    	this->bg = t_bg;
    };
	...
	// Drawing lines between points instead of just points
        while(x < y)
        {
            ...
            ddF_x += 2;
            f += ddF_x + 1;
            Line line1({x0+x, y0+y}, {x0-x, y0+y}, fg, bg);
            line1.draw();
            Line line2({x0+x,y0-y},{x0-x,y0-y},fg,bg);
            line2.draw();;
            Line line3({x0+y,y0+x},{x0-y,y0+x},fg,bg);
			line3.draw();;
...
public:
    // Center of triangle
    Point2D center;
    // Length o            //drawPixel(x0 + x, y0 + y);
    //drawPixel(x0 - x, y0 + y);f one side of triangle
    int strana;
    // Colours of triangle
    RGB fg, bg;
    // Height of triangle
    double vyska;
    //
    Point2D point_c1, point_c3, point_a1;
        ...
        center( t_center ), strana( t_strana ), GraphElement( t_fg, t_bg )
    {
    	...
    	this->point_a1 = {center.x, center.y - (vyska / 2)};
